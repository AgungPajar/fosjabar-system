<x-app-layout>
    <x-slot name="header">
        <div class="flex w-full flex-col gap-3">
            <h1 class="text-3xl font-semibold text-slate-900 dark:text-white">Tambah Generasi</h1>
    ...
    <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <h2 class="text-xl font-semibold text-slate-900 dark:text-white">Form Generasi</h2>
        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Lengkapi informasi generasi baru.</p>
        ...
        <form action="{{ route('generations.store') }}" method="POST" class="mt-6 space-y-6">
            @csrf
            ...
            <div class="grid gap-6 md:grid-cols-2">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="singkatan" value="{{ old('singkatan') }}" required class="w-full rounded-lg border border-slate-300 px-4 py-3 text-sm text-slate-900 shadow-sm transition focus:border-indigo-500 focus:outline-none focus:ring focus:ring-indigo-100 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 dark:focus:border-indigo-400">
                </div>
            </div>
            ...
            <div class="grid gap-6 md:grid-cols-2">
                <div class="space-y-2">
                    ...
                    <input type="date" name="started_at" value="{{ old('started_at') }}" required class="w-full rounded-lg border border-slate-300 px-4 py-3 text-sm text-slate-900 shadow-sm transition focus:border-indigo-500 focus:outline-none focus:ring focus:ring-indigo-100 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 dark:focus:border-indigo-400">
                </div>
                <div class="space-y-2">
                    <label class="text-sm font-medium text-slate-700 dark:text-slate-300">Tanggal Selesai</label>
                    <input type="date" name="ended_at" value="{{ old('ended_at') }}" required class="w-full rounded-lg border border-slate-300 px-4 py-3 text-sm text-slate-900 shadow-sm transition focus:border-indigo-500 focus:outline-none focus:ring focus:ring-indigo-100 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 dark:focus:border-indigo-400">
                </div>
            </div>
            ...
            <div class="space-y-2">
                ...
                <select name="participants_id" class="w-full rounded-lg border border-slate-300 px-4 py-3 text-sm text-slate-900 shadow-sm transition focus:border-indigo-500 focus:outline-none focus:ring focus:ring-indigo-100 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 dark:focus:border-indigo-400">
                    ...
                    @foreach ($participants as $participant)
                        <option value="{{ $participant->id }}" @selected(old('participants_id') === $participant->id)>
                            {{ $participant->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <label class="inline-flex items-center gap-3">
                <input type="checkbox" name="is_active" value="1" @checked(old('is_active', true)) class="h-5 w-5 rounded border-slate-300 text-indigo-600 focus:ring focus:ring-indigo-200 dark:border-slate-700 dark:bg-slate-800 dark:text-indigo-400">
